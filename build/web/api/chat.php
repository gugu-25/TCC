...
foreach (array_slice($mensagens, -10) as $mensagem) {
    ...
    if (in_array($role, ["user", "assistant"], true) && $content !== "") { $validas[] = ["role" => $role, "content" => $content]; }
}
...
if ($status < 200 || $status >= 300 || empty($dados["choices"][0]["message"]["content"])) { http_response_code(502); echo json_encode(["erro" => "Resposta invalida da OpenAI.", "detalhe" => $dados["error"]["message"] ?? null]); exit; }
